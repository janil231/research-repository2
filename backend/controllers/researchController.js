...
exports.servePDF = async (req, res) => {
  try {
    ...
    // Resolve possible file locations (handle relative paths and basename fallback)
    const rawPath = research.pdfFilePath;
    const candidates = [];

    // If absolute, try as-is
    if (path.isAbsolute(rawPath)) {
      candidates.push(rawPath);
    }

    // Relative to project root
    candidates.push(path.join(ROOT_DIR, rawPath));

    // Fallback: uploads/research + basename
    candidates.push(path.join(ROOT_DIR, 'uploads', 'research', path.basename(rawPath)));
