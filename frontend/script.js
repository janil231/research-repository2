...
// Define API base URL - works both locally (port 5000) and in production (same origin)
function getApiBaseUrl() {
  const port = window.location.port;
  // If serving on port 5000 or standard HTTP(S) ports, API is same origin
  if (port === "5000" || port === "" || port === "80" || port === "443") {
    return "";
  }
  // Local dev on different port (e.g. 5500) -> API is on port 5000
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Hamburger button: sidebar toggle (admin & student dashboards)
  ...
  if (adminChangePasswordForm) {
    adminChangePasswordForm.addEventListener("submit", async function (e) {
      ...
      const confirmNewPassword = document
        .getElementById("adminConfirmNewPassword")
        .value.trim();
  ...
  const openCreateAdminBtn = document.querySelector(
...
if (registerForm) {
  ...
  registerForm.addEventListener("submit", async function (e) {
    ...
    if (role === "student") {
      ...
      const studentTermsCheckbox = document.getElementById(
        "studentTermsCheckbox",
      );
      console.log(
        "Student terms checkbox:",
        studentTermsCheckbox,
        "Checked:",
        studentTermsCheckbox ? studentTermsCheckbox.checked : "not found",
      );
      ...
      const favoriteColorValue =
        document.getElementById("adminFavoriteColor").value;
      payload = {
        role: "admin",
...
function showAccountType(type) {
  ...
  const targetTab = document.querySelector(
    `button[onclick="showAccountType('${type}')"]`,
  );
  if (targetTab) {
...
async function approveResearch(researchId, status) {
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(
      `${API_BASE_URL}/api/research/${researchId}/approve`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  if (document.getElementById("uploadWizardForm")) {
    ...
    researchForm.addEventListener("submit", async function (e) {
      ...
      try {
        ...
        showFormError(
          researchForm,
          error.message || "Failed to upload research. Please try again.",
        );
      } finally {
        submitBtn.disabled = false;
        submitBtn.textContent = originalBtnText;
...
function showQRCodeModal(qrCodeDataUrl, secret, username) {
  ...
  overlay.style.cssText =
    "position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 10000; display: flex; align-items: center; justify-content: center; overflow-y: auto; padding: 20px;";
...
async function performSearch() {
  ...
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(
      `${API_BASE_URL}/api/research/search?${params.toString()}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (toggleAdvancedBtn && advancedFilters) {
    toggleAdvancedBtn.addEventListener("click", () => {
      ...
      toggleAdvancedBtn.textContent = visible
        ? "+ Add filters"
        : "− Hide filters";
    });
  }
...
if (forgotPasswordForm) {
  ...
  forgotPasswordForm.addEventListener("submit", async function (e) {
    ...
    if (role === "admin") {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/reset/request-reset`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (response.ok) {
          ...
          showFormError(
            forgotPasswordForm,
            data.message || "Failed to submit reset request.",
          );
        }
      } catch {
        showFormError(forgotPasswordForm, "Could not connect to server.");
...
async function handleResetRequest(requestId, status) {
  try {
    ...
    const requestResponse = await fetch(
      `${API_BASE_URL}/api/reset/reset-requests`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/reset/reset-request/${requestId}`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
        ...
        body: JSON.stringify({
          status,
          newPassword:
            status === "approved" ? request.userId.schoolId : undefined,
        }),
      },
    );
...
async function loadStatistics() {
  try {
    ...
    console.log(
      ...
      `${API_BASE_URL}/api/statistics/overall`,
    );
    console.log("Headers being sent:", headers);

    // Fetch all statistics in parallel
    const [overallStats, departmentStats, yearlyStats, trendStats] =
      await Promise.all([
        fetch(`${API_BASE_URL}/api/statistics/overall`, { headers }).then(
          (res) => {
        ...
        fetch(`${API_BASE_URL}/api/statistics/by-department`, { headers }).then(
          (res) => {
            if (!res.ok)
              throw new Error(`Department API returned ${res.status}`);
            return res.json();
          },
        ),
        fetch(`${API_BASE_URL}/api/statistics/by-year`, { headers }).then(
          (res) => {
            if (!res.ok) throw new Error(`Year API returned ${res.status}`);
            return res.json();
          },
        ),
        fetch(`${API_BASE_URL}/api/statistics/trends`, { headers }).then(
          (res) => {
    ...
    async function redrawDepartmentFromFilters() {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/statistics/by-department${params.toString() ? "?" + params.toString() : ""}`,
        {
      ...
      const toAcr = (name) => {
        ...
        const key = Object.keys(departmentAcronymMap).find(
          (dep) => dep.trim().toLowerCase() === name.trim().toLowerCase(),
        );
        if (key) return departmentAcronymMap[key];
        // Try substring match as fallback (partial name inside key)
        const partial = Object.keys(departmentAcronymMap).find((dep) =>
              ...
              callbacks: {
                ...
                label: function (context) {
                  ...
                  const pct = Math.round(
                    (values[idx] / (values.reduce((a, b) => a + b, 0) || 1)) *
                      100,
                  );
    ...
    document
      .getElementById("chartDepartmentYear")
      ?.addEventListener("change", redrawDepartmentFromFilters);
    document
      .getElementById("chartDepartmentSemester")
      ?.addEventListener("change", redrawDepartmentFromFilters);
    ...
    if (trendChart) {
      ...
      window.trendChartInstance = new Chart(trendCtx, {
        type: "line",
        data: {
          labels: trendStats.map(
            (stat) => `${stat._id.year}-${stat._id.month}`,
          ),
          datasets: [
            {
...
async function exportStatistics(format) {
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(
      `${API_BASE_URL}/api/statistics/export?format=${format}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
async function submitWizard(e) {
  ...
  const authorsVal = (
    document.getElementById("wizardAuthors")?.value || ""
  ).trim();
  const keywordsVal = (
    document.getElementById("wizardKeywords")?.value || ""
  ).trim();
  ...
  if (
    ...
    alert(
      "Please complete all required fields before uploading (Title, Authors, Adviser(s), Department, Year, Semester).",
    );
    return;
  }
...
async function exportPdfReport() {
  ...
  const semester =
    document.getElementById("chartDepartmentSemester")?.value || "";
  let query = "";
...
async function downloadBackup() {
  ...
  try {
    ...
    if (btn) {
      ...
      btn.innerHTML =
        '<i class="fas fa-spinner fa-spin" style="margin-right: 0.5rem;"></i> Downloading...';
    }
    if (loadingEl) loadingEl.style.display = "inline-block";
    const token = localStorage.getItem("token");
    if (!token) {
      showNotification(
        "You must log in first, then try downloading again.",
        "error",
      );
    ...
    // Use direct browser download with token as query param (also accepted by backend)
    const downloadUrl = `${API_BASE_URL}/api/research/backup?token=${encodeURIComponent(token)}`;
    const a = document.createElement("a");
    a.href = downloadUrl;
    ...
    a.click();
    a.remove();
    showNotification("Research backup (ZIP) download started.", "success");
  } catch (error) {
    console.error("downloadBackup error:", error);
    showNotification(
      error.message || "Failed to download backup. Please try again.",
      "error",
    );
  } finally {
    if (btn) {
...
async function restoreFromBackup(inputEl) {
  ...
  try {
    ...
    if (btn) {
      ...
      btn.dataset.origHtml = btn.innerHTML;
      btn.innerHTML =
        '<i class="fas fa-spinner fa-spin" style="margin-right: 0.5rem;"></i> Restoring...';
    }
    if (loadingEl) loadingEl.style.display = "inline-block";
    ...
    if (!res.ok) {
      const msg =
        data && data.message ? data.message : "Failed to restore backup";
      throw new Error(msg);
    }

    const s = data.summary || {};
    showNotification(
      `Restore completed: ${s.created || 0} created, ${s.updated || 0} updated, ${s.skippedNoFile || 0} skipped.`,
      "success",
    );
...
async function clearAllResearch() {
  ...
  try {
    const confirmText =
      "This will delete ALL research records. This action cannot be undone. Continue?";
    const ok = window.confirm(confirmText);
    if (!ok) return;

    const keepFiles = !!(
      document.getElementById("keepFilesCheckbox") &&
      document.getElementById("keepFilesCheckbox").checked
    );
    const token = localStorage.getItem("token");
    ...
    if (btn) {
      ...
      btn.dataset.origHtml = btn.innerHTML;
      btn.innerHTML =
        '<i class="fas fa-spinner fa-spin" style="margin-right: 0.5rem;"></i> Clearing...';
    }
    if (loadingEl) loadingEl.style.display = "inline-block";

    const res = await fetch(
      `${API_BASE_URL}/api/research?keepFiles=${keepFiles ? "true" : "false"}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (!res.ok) {
      const msg =
        data && data.message ? data.message : "Failed to clear research";
      throw new Error(msg);
    }
    ...
    showNotification(
      `Cleared ${deletedCount} records${keepFiles ? "" : `, deleted ${filesDeleted} files`}.`,
      "success",
    );
  } catch (err) {
...
async function archiveResearch(researchId) {
  ...
  const ok = window.confirm(
    "Are you sure you want to archive this research? You can view archived items in Settings → Archived Research.",
  );
  if (!ok) return;
  try {
    const token = localStorage.getItem("token");
    const res = await fetch(
      `${API_BASE_URL}/api/research/${researchId}/archive`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
...
async function openArchived() {
  try {
    ...
    const data = await res.json();
    if (!res.ok)
      throw new Error(data.message || "Failed to fetch archived research");
...
async function restoreResearch(researchId) {
  if (
    !confirm(
      "Are you sure you want to restore this research? It will be visible in the main search again.",
    )
  ) {
    return;
  }
  ...
  try {
    const token = localStorage.getItem("token");
    const res = await fetch(
      `${API_BASE_URL}/api/research/${researchId}/restore`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
...
async function loadTopAuthors() {
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/statistics/top-authors?limit=${limit}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
    ...
    container.innerHTML = data
      .map(
        (item, idx) =>
          `<div class="stat-row"><span class="stat-rank">${idx + 1}</span><span class="stat-label">${item.author}</span><span class="stat-value">${item.count}</span></div>`,
      )
      .join("");
  } catch (error) {
...
async function loadTopKeywords() {
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/statistics/top-keywords?limit=${limit}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
    ...
    container.innerHTML = data
      .map(
        (item, idx) =>
          `<div class="stat-row"><span class="stat-rank">${idx + 1}</span><span class="stat-label">${item.keyword}</span><span class="stat-value">${item.count}</span></div>`,
      )
      .join("");
  } catch (error) {
...
document
  .getElementById("topAuthorsLimit")
  ?.addEventListener("change", loadTopAuthors);
document
  .getElementById("topKeywordsLimit")
  ?.addEventListener("change", loadTopKeywords);
...
function initializeStudentSearch() {
  ...
  function renderTagList() {
    tagsEl.innerHTML = tags
      .map(
        (t, i) =>
          `<span class="tag-chip">${t}<span class="remove" data-i="${i}">×</span></span>`,
      )
      .join("");
  }
      ...
      .map((p) => {
        const authors = (
          Array.isArray(p.authors)
            ? p.authors
            : String(p.authors || "").split(",")
        )
          .filter(Boolean)
          .map((a) => String(a).trim());
        const advisers = String(p.adviser || "")
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean);
...
document.addEventListener("DOMContentLoaded", function () {
  if (
    ...
    document.getElementById("profileJoined").value = joinDate
...
function updateAllAnalytics() {
  ...
  fetch(`/api/statistics/by-department${query}`, { headers })
    .then((r) => r.json())
    .then((deps) => {
      const totalCount = Array.isArray(deps)
        ? deps.reduce((sum, d) => sum + d.count, 0)
        : 0;
...
async function exportPdfReport() {
  ...
  const year = document.getElementById("analyticsYearFilter")?.value || "";
  const semester =
    document.getElementById("analyticsSemesterFilter")?.value || "";
  let query = "";
...
if (
  document.getElementById("analyticsYearFilter") &&
...
if (
  ...
  createAdminForm.addEventListener("submit", async function (e) {
    ...
    const favoriteColorValue =
      document.getElementById("adminFavoriteColor")?.value.trim() || "";
    const formData = {
...
function attachPasswordToggleListeners(scope) {
  (scope
    ? scope.querySelectorAll(".toggle-password-visibility")
    : document.querySelectorAll(".toggle-password-visibility")
  ).forEach((btn) => {
